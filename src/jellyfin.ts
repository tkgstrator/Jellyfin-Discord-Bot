...
export class JellyfinClient {
  ...
  async getPlaylistItems(playlistId: string): Promise<MediaItem[]> {
    try {
      ...
      return items.map((item) => {
        ...
        const durationSeconds = item.RunTimeTicks ? Math.floor(item.RunTimeTicks / 10_000_000) : null
  ...
  /**
   * ストリーミングURLを生成
   * Opus形式にトランスコードしてストリーミング
   */
  private getStreamUrl(itemId: string, _mediaType: MediaType): string {
    // Opusコーデック、WebMコンテナでトランスコード
    // Discord.jsはWebM/Opus形式を最適にサポート
    const params = new URLSearchParams({
      ...
      maxStreamingBitrate: '128000' // 128kbps (Discordの制限)
    })
    return `${this.config.serverUrl}/Audio/${itemId}/universal?${params.toString()}`
  }
